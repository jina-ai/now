...
def test_text_index_fails_with_no_flow_running(client: requests.Session):
    with pytest.raises(ConnectionError):
        client.post(f'/api/v1/text-to-image/index', json={'texts': ['Hello']})
...
def test_text_index(
    ...
    response = client_with_mocked_jina_client(DocumentArray()).post(
        '/api/v1/text-to-image/index', json={'texts': ['Hello']}
    )
...
def test_text_search_parse_response(
    ...
    assert response.status_code == status.HTTP_200_OK
    results = DocumentArray.from_json(response.content)
    assert len(results) == len(sample_search_response_text)
    assert results[0].text == sample_search_response_text[0].text
